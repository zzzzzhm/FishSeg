...
output_folder = 'fish_dateset/bright'
...
def place_fish_mask_on_background(background, mask, position):
    """
    将鱼图片放置到背景上的指定位置
    :param background: 背景图片张量 (C, H, W)
    :param fish_img: 鱼图片张量 (C, h, w)
    ...
    """
    x, y = position
    h, w = mask.shape[1:]  # 鱼图片的高度和宽度
    tensor = torch.zeros(background.size())

    tensor[:, y:y+h, x:x+w] = (
        tensor[:, y:y+h, x:x+w] * (1 - mask) + mask
    )
    return tensor
...
for fish_type in os.listdir(dataset_folder):
    ...
    for original_name in tqdm(os.listdir(original_folder), desc=f"Processing {fish_type}"):
        ...
        binary_mask = place_fish_mask_on_background(background_img.clone(), binary_mask, (x, y))


        # # 生成随机噪声强度（标准差）
        # random_std = random.uniform(0.01, 0.2)  # 随机生成 0.01 到 0.2 的标准差
        # fish_img = add_gaussian_noise(composite_img, mean=0, std=random.uniform(0.01, 0.2))

        fish_img = adjust_brightness_tensor(composite_img, factor=random.uniform(1.2, 1.8))
